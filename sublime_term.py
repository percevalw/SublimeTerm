...
class SublimetermCommand(sublime_plugin.WindowCommand):
	def run(self, make_new=False, key = None, **kwargs):
		...
		print("\n############ BEGINNING SUBLIME CONNECT ############")
		print("NEW", make_new)

		# RELOADING SETTINGS
		self.settings = sublime.load_settings("SQLQuery.sublime-settings")
...
class SublimetermEditorCommand(sublime_plugin.TextCommand):
	def run(self, edit, action = 0, begin = 0, end = 0, string = ""):
		if action == 0:
...
#			print("EDITOR REPLACE", repr(string), "AT", begin, end)
